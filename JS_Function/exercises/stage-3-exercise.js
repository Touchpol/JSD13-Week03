...
function findMenuItemById(items, itemId) {return items.find((item)=>items.id===itemId);}
//items.findเป็นการสั่งให้ array หาข้อมูล, (item)=> เป็นการสั่งให้ดูข้อมูลทีละตัว(มีตัวแปรคือitem), itemID ตรงตัวหาด้วยไอดี
...
function findMenuItemByName(items, itemName) {return items.find((item)=>item.name===itemName);}
//items.find()เป็นการสั่งให้ array หาข้อมูล, (item)=> เป็นการสั่งให้ดูข้อมูลทีละตัว(มีตัวแปรคือitem), item.name===itemName คือเงื่อนไขการเชค ถ้าnameของออเดอร์ตรงกับitemNameที่ต้องการ ฟังก์ชั่นจะส่งออเดอรืกลับมาให้เรา
...
function findOrderById(orderList, orderId) {return orderList.find((order)=>order.id===orderId);}
//orderList.find()เป็นการสั่งให้ array หาข้อมูล, (order)=> เป็นการสั่งให้ดูข้อมูลทีละตัว(มีตัวแปรคือorder), order.id===orderID คือเงื่อนไขการเชค ถ้าidของออเดอร์ตรงกับorderIdที่ต้องการ ฟังก์ชั่นจะส่งออเดอรืกลับมาให้เรา
...
// Expected outputs:
//   getMenuItemNameById(menuItems, 2)  → "Double Engine" (getMenuItemNameById=item.id===itemID)
//   getMenuItemNameById(menuItems, 99) → "Menu item not found"

function getMenuItemNameById(items, itemId) {const foundItem=items.find((item)=>item.id===itemId);
    if (foundItem){return foundItem.name;}
    else{return"Menu item not found";}
}
//ถ้าไม่มีreturnข้างหน้า จะถูกมองว่า"foundItem" คือการประกาศตัวแปร แล้วตัวแปลต้องมี (const, let) = ต้องมี const/let อยู่ข้างหน้าทุกครั้งเพื่อประกาศตัวแปร
