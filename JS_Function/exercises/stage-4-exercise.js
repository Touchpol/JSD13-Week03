...
function getUnavailableItems(items) {return items.filter((item)=>item.isAvailable===false);}
//items.filter()สั่งให้โปรแกรมหารายการอาหารทั้งหมดในitems, (item)=> เป็นการสั่งให้ดูข้อมูลทีละตัว(มีตัวแปรคือitem), item.isAvailable===falseเป็นเงื่อนไขการตรวจสอบ(อันไหนเป็นtrueจะถูกข้ามไป)
...
function getAffordableItems(items) {return items.filter((item)=>item.price<200);}
//items.filter()สั่งให้โปรแกรมหารายการอาหารทั้งหมดในitems, (item)=> เป็นการสั่งให้ดูข้อมูลทีละตัว(มีตัวแปรคือitem), item.price<200เป็นเงื่อนไขการตรวจสอบเพดานราคา
...
function getLowStockItems(items) {return items.filter((item)=>item.stock<10);}
...
function getCompletedOrders(orderList) {return orderList.filter((order)=>order.status==="completed");}
//orderList.filter, status==="completed"ใช้หาออเดอร์ที่มีสถานะเป็นcompleted
...
function getAvailableBurgers(items) {return items.filter((item)=>item.category==="burger"&&item.isAvaliable===true);}
//มีเงื่อนไข2ข้อต้องใช้ "and" แต่ภาษาโค้ตใช้ "&&"
...
function getBurgersWithinBudget(items, maximumPrice) {return items.filter((item)=>item.category==="burger"&&item.price<=maximumPrice);}
//getBurgersWithinBudget โจทย์มีเงื่อนไขซ้อนกันคือต้องเป็นburgerและราคาน้อยกว่าหรือเท่ากับmaximumPrice ต้องใช้ "burger"&&item"และ price<=(เครื่องหมายน้อยกว่าเท่ากับในภาษาคอม)maximumPrice 
